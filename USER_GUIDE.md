...
选择 Bitcoin 五分钟后，可以随时按 `Tab` 切换方向。面板会原地更新当前方向、缩写 Token、`bestBid`、`bestAsk`、平均买入价和自动卖出目标价。切换不需要重新连接，并会关闭当前自动卖出 armed 状态；需要自动卖出时再按一次 `a`。按 `←` 或 `→` 可手动尝试切换到上一期或下一期；如果目标场次不存在、未开放或已结束，程序会提示无法切换并保留当前连接。当前场次结束后程序仍会自动切换。
